...
Obviously, with any data analysis it's important to understand the data you're using. To understand the timeframe of our dataset, we simply graphed a simple histogram of the dates that the articles were published.
...
The simplest way ~~to do this~~ was to simply take the top 5 words from both the fake news and real news categories and examine both their individual frequencies and the distribution among the top 5.
...
# sentiment: entiment label, "value" for affin and "sentiment" for nrc
...
plot_sentiments(get_freq(sentiment_affin, "value"), "AFFIN Sentiments") + 
  scale_x_continuous(limits=c(-5,5), breaks=seq(-5,5,1))

plot_sentiments(get_freq(abs, "value"), "AFFIN Sentiments (Absolute Values)") + 
...
Basic Sentiment Analysis (similar to what we did with the text of the articles)
```{r titlesentiment}

title_affin = titles %>%
  inner_join(get_sentiments("afinn"))
plot_sentiments(get_freq(title_affin, "value"), "Title AFFIN Sentiments")

title_abs = title_affin %>%
  mutate(value = abs(value))
plot_sentiments(get_freq(title_abs, "value"), "Title AFFIN Sentiments (Absolute Values)")
...
```{r combined_graphs}
plot_combined_sentiments(title_vs_article(title_affin, sentiment_affin, "value"), 
                         "Title vs. Article AFFIN Sentiment") +
  scale_x_continuous(limits=c(0,5), breaks=seq(0,5,1))

plot_combined_sentiments(title_vs_article(title_abs, abs, "value"), 
                         "Title vs. Article AFFIN Sentiment (Absolute Values)") + 
  scale_x_continuous(limits=c(0,5), breaks=seq(0,5,1))
